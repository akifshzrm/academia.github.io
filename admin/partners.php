...
if ($op == 'delete') {
    ...
    $sql1   = "select photos from partners where id = '$id'";
...
<h1>Admin Page Partners</h1>
<p>
    <a href="partners_input.php">
        <input type="button" class="btn btn-primary" value="Add New Partners"/>
    </a>
</p>
...
<tbody>
    ...
    $sql1 = " select * from partners $sqladd";
    ...
    ?>
        <tr>
        <td><?php echo $num++ ?></td>
        <td><img src="../images/<?php echo partners_photo($r1['id'])?>" style="max-height:100px;max-width:100px"/></td>
        <td><?php echo $r1['name'] ?></td>
        <td>
        <a href="partners_input.php?id=<?php echo $r1['id']?>">
        <span class="badge bg-warning text-dark">Edit</span>
        </a>
        <a href="partners.php?op=delete&id=<?php echo $r1['id']?>" onclick="return confirm('Confirm on delete?')">
...
<nav aria-label="Page navigation example">
    <ul class="pagination">
        ...
        for($i=1; $i <= $pages; $i++){
            ?>
            <li class="page-item">
                <a class="page-link" href="partners.php?keywords=<?php echo $keywords?>&find=<?php echo $find?>&page=<?php echo $i ?>"><?php echo $i ?></a>
            </li>
            <?php
        }
        ?>

...
if (isset($_POST['save'])) {
    ...
    if (empty($error)) {
        if($photos_name){
            ...
            $photos_name = "partners_".time()."_".$photos_name;
            move_uploaded_file($photos_file,$directory."/".$photos_name);

            $photos = $photos_name;
        }else{
            $photos_name = $photos; //memasukkan data dari data yang sebelumnya ada
        }

        if($id != ""){
            $sql1   = " update partners set name = '$name',photos='$photos_name',content='$content',date_content=now() where id = '$id'";
        }else{
            $sql1       = "insert into partners(name,photos,content) values ('$name','$photos_name','$content')";
        }
